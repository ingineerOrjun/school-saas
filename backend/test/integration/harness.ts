...
export async function startIntegrationDb(): Promise<PrismaClient> {
  if (!isDockerAvailable()) {
    // Operator-facing error message that says WHAT failed, WHY it
    // failed, and WHAT TO DO NEXT — same standard the rest of the
    // codebase follows (see OPERATOR_FAILURE_SCENARIOS.md).
    throw new Error(
      [
        'Integration test harness requires Docker but the `docker` CLI is not on PATH.',
        'Install Docker Desktop (Windows / macOS) or the docker engine (Linux), start the daemon, and re-run.',
        'On a CI runner without Docker, wrap your describe block in `describeWithDb(...)` so the suite skips cleanly instead of failing.',
        'Verified harness behaviour: `npm run test:integration` exits 0 with all suites skipped when Docker is absent.',
      ].join(' '),
    );
  }
...
function applyMigrations(url: string): void {
  ...
  // `prisma migrate deploy` is non-interactive (it never prompts) and
  // matches production. We deliberately do NOT call `prisma generate`
  // here — the generated client is committed to node_modules and is
  // re-generated by `npm install`. A clean integration runner should
  // run `npm install` first; if `@prisma/client` is missing we surface
  // a clearer error from PrismaClient's constructor than from a
  // generate-during-test invocation.
